...
export const MatchReasons: React.FC<MatchReasonsProps> = ({ reasons }) => {
  ...
  return (
    <div className="p-4 rounded-xl bg-[#173D32]/10 border border-[#173D32]/25 space-y-2 shadow-2xs">
      <div className="flex items-center gap-2 text-xs font-mono font-bold text-[#173D32] uppercase tracking-wider">
        <CheckCircle2 className="size-4" /> Primary Match Compatibility Drivers
      </div>
      <ul className="space-y-1.5 text-xs text-[#171A18] font-sans">
        {reasons.map((r, i) => (
          <li key={i} className="flex items-start gap-2">
            <span className="text-[#173D32] font-bold">•</span>
            <span>{r}</span>
          </li>
        ))}
      </ul>
